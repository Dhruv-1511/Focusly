      ...
      {menuGroups.map((group) => (
        <div key={group.label} className="space-y-2">
          <div className="px-4 mb-4">
             <div className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.3em]">{group.label}</div>
          </div>
            ...
            {group.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={onClose}
                  className={cn(
                    "flex items-center gap-4 px-4 py-3 transition-all duration-300 group relative rounded-2xl overflow-hidden",
                    isActive 
                ...
                >
                  ...
                  <span className={cn(
                    "relative z-10 text-sm font-bold tracking-tight transition-colors duration-300",

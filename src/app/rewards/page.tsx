  ...
  return (
    <div className="p-8 lg:p-12 max-w-7xl mx-auto">
      <header className="mb-12">
        <h1 className="text-4xl font-black mb-2 italic text-primary">Hall of Achievement</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-12">
           {/* Progress Section */}
           <section className="bg-card rounded-[40px] border shadow-sm p-8 md:p-12 relative overflow-hidden">
              <div className="flex flex-col md:flex-row justify-between md:items-end gap-8 mb-12">
                 <div>
                    <div className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-4">Current Progress</div>
                    <div className="flex items-baseline gap-2">
                       <span className="text-7xl font-black italic tracking-tighter">Level 15</span>
                       <span className="text-xl font-bold text-primary italic">Professional Student</span>
                    </div>
                 </div>
                 <div className="text-right">
                    <div className="text-sm font-bold italic mb-1">XP to Level 16</div>
                    <div className="text-2xl font-black italic text-primary">450 / 3000 XP</div>
                 </div>
              </div>
           ...
           <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                 {badges.map((badge, i) => (
                   <div key={i} className={`p-8 rounded-[32px] border transition-all group relative overflow-hidden ${badge.locked ? 'bg-muted/30 grayscale opacity-60' : 'bg-card hover:shadow-xl hover:border-primary'}`}>
                      <div className="text-5xl mb-6 relative z-10">{badge.icon}</div>
                      <h3 className="text-lg font-black mb-2 italic relative z-10">{badge.name}</h3>
        ...
        <div className="space-y-8">
           {/* Daily Challenges */}
           <div className="bg-foreground text-background rounded-[40px] p-8 shadow-xl">
              <h3 className="text-xl font-bold mb-6 italic text-white flex items-center gap-2">
              ...
              <Button className="w-full mt-8 h-12 bg-primary text-white font-black italic rounded-2xl">
                 Reset in 4h 22m
              </Button>
           </div>
           ...
           <div className="bg-card rounded-[32px] border p-8 shadow-sm">
              <h3 className="text-xl font-bold mb-6 italic">Top Academy Rank</h3>
              ...
              <Button variant="outline" className="w-full rounded-xl flex items-center gap-2 font-bold italic">
                 View Full Leaderboard <ArrowRight className="h-4 w-4" />
              </Button>
           </div>

           {/* Prize Box */}
           <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-[32px] p-8 text-white relative overflow-hidden group cursor-pointer">
              <Crown className="h-10 w-10 mb-4" />
              <h3 className="text-xl font-black italic mb-2">Mystery Reward Box</h3>
  ...
  return (
    <div className="p-4 bg-white/5 rounded-2xl border border-white/10 group hover:border-white/30 transition-colors">
       <div className="flex justify-between items-center mb-2">
          ...
          <div className="text-xs font-black text-primary">{xp} XP</div>
       </div>
...
function CheckIcon() {
  return <div className="h-4 w-4 bg-secondary rounded-full flex items-center justify-center text-[10px] text-white">✓</div>;
}

function LeaderItem({ rank, name, xp, active }: any) {
  return (
    <div className={`flex items-center justify-between p-3 rounded-xl ${active ? 'bg-primary/10 border border-primary text-primary' : 'bg-muted/30'}`}>
       <div className="flex items-center gap-3">
          <span className="text-xs font-black text-muted-foreground w-4">{rank}</span>

    ...
    <div className="p-8 lg:p-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
           ...
           <AnimatePresence mode="wait">
              {toolMode ==="revision" ? (
                 <motion.section 
                    ...
                    className="space-y-8"
                 >
                    <div className="bg-card rounded-3xl border-2 border-primary/20 p-8 md:p-12 shadow-sm">
                       ...
                       <div className="space-y-4">
                          {revisionTopics.map((topic, i) => (
                             <div key={i} className="flex items-center gap-6 p-6 rounded-[28px] bg-muted/30 border-2 border-transparent hover:border-border transition-all group">
                                ...
                                <Button className={`h-12 px-8 rounded-2xl font-bold  ${topic.status === 'Urgent' ? 'bg-red-500 hover:bg-red-600' : 'bg-primary'}`}>
                                   Revise Now
                                </Button>
                             </div>
                          ))}
                ...
                <motion.section 
                  ...
                  className="space-y-8"
                >
                   <div className="bg-card rounded-3xl border-2 border-dashed border-primary/20 p-12 md:p-20 flex flex-col items-center justify-center text-center">
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 w-full max-w-lg">
                         <Button onClick={handleUpload} disabled={isUploading} size="lg" className="h-16 flex-1 rounded-2xl bg-foreground text-background font-bold  text-lg">
                            {isUploading ?"Reading Text..." :"Choose File"}
                         </Button>
                         <Button variant="outline" size="lg" className="h-16 px-8 rounded-2xl font-bold">Paste Content</Button>
                      </div>
                      {isUploading && (
        ...
        <div className="space-y-8">
           {/* Progress Widget */}
           <div className="bg-secondary text-white rounded-3xl p-8 shadow-sm relative overflow-hidden">
              <h3 className="text-xl font-bold mb-6">Memory Strength</h3>
              <div className="flex items-center gap-6 mb-8">
                 <div className="text-6xl font-bold tracking-tighter">84%</div>
                 <div className="text-xs uppercase font-bold tracking-widest text-white/70">Overall<br/>Retention</div>
              </div>
              <div className="space-y-4">
              ...
              <Button className="w-full mt-8 bg-white text-secondary font-bold">View Mastery Map</Button>
              <ListRestart className="absolute bottom-[-10px] right-[-10px] h-32 w-32 text-white/10 rotate-12" />
           </div>

           {/* Tip Card */}
           <div className="bg-card rounded-2xl border p-8">
              <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground mb-4">Scientific Fact</h3>

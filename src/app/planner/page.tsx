          ...
          {step === 1 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                {subjects.map((sub, i) => (
                  <div key={i} className="flex items-center gap-3 p-4 rounded-2xl bg-muted/50 border group">
                    <span className="font-bold flex-1">{sub}</span>
                    <button onClick={() => setSubjects(subjects.filter((_, idx) => idx !== i))} className="text-muted-foreground hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <Button onClick={() => setStep(2)} className="w-full h-14 bg-primary text-white font-bold text-lg rounded-2xl">
